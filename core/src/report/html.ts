...
<div class="foot">
  ...
  <p><b>Methodology:</b> all figures are estimates. Percentages are of total spend; "addressable waste" is the sum of findings, separate from base spend. Monthly projections extrapolate the ${result.daysInDataset}-day period to 30 days. Every threshold and formula is documented in <a href="https://github.com/abhinavagg810/token_triage/blob/main/core/src/analyzers/THRESHOLDS.md">THRESHOLDS.md</a>. A claimed-token ledger guarantees no token is counted by two analyzers.</p>
  ${result.unverifiedPricing.length > 0 ? `<p><b>⚠ Unverified pricing:</b> bundled prices for ${esc(result.unverifiedPricing.join(", "))} are placeholders (<code>last_verified: UNVERIFIED</code>). Verify them against the provider pricing pages or set your rates in <code>~/.tokentriage/pricing.override.json</code> before relying on the dollar figures.</p>` : ""}
